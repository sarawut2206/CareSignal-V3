...
(function (g) {
  ...
  var KIND = { ftsst: 1, tug: 2, balance: 3 }, KIND_OF = { 1: "ftsst", 2: "tug", 3: "balance" };
  var CMD = { ARM: 1, GO: 2, STOP: 3, STREAM: 4, RESET: 5 };
  ...
  function makeState(kind, stage) {
    return { kind: kind, stage: stage || 0, phase: "idle", n: 0, tPrev: null, fsSum: 0, g: null, v0: null,
      calN: 0, calSum: [0, 0, 0], calMag: 0, calMove: 0, holds: 0,
  ...
  function arm(st, t) { st.phase = "cal"; st.calN = 0; st.calSum = [0, 0, 0]; st.calMag = 0; st.calMove = 0; st.tPrev = null; }
  ...
  function push(st, t, a, w) {
    ...
    if (st.phase === "cal") {
      ...
      if (st.calN >= P.CAL_N) {
        if (st.calMove < P.CAL_W) { st.v0 = unit(st.calSum); st.g = st.v0.slice(); st.avB = st.calMag / st.calN - 1; st.phase = "ready"; ev = EV.READY; }   /* avB = ค่าคลาดของสเกลความเร่งขณะนิ่ง */
        else { st.calN = 0; st.calSum = [0, 0, 0]; st.calMag = 0; st.calMove = 0; st.holds++; ev = EV.HOLD; }
      }
  ...
  function stop(st, t) { if (st.phase === "run" && !st.done) finish(st, t, st.kind === "balance" ? "ok" : "incomplete"); else if (!st.done) { st.done = true; st.phase = "done"; st.result = { kind: st.kind, engine: ENGINE, status: "aborted" }; } return st.result; }
  ...
  function pack(res) {
    ...
    dv.setUint8(0, 0xC5); dv.setUint8(1, 1); dv.setUint8(2, KIND[res.kind] || 0); dv.setUint8(3, { ok: 0, incomplete: 1, aborted: 2 }[res.status] || 0);
    dv.setUint8(4, f.reps || 0); dv.setUint8(5, (res.impact ? 1 : 0) | (bl.stepped ? 2 : 0) | (tg.turnSeen ? 4 : 0));
    ...
    dv.setUint16(48, u16(tg.turnDeg, 10), true); dv.setUint16(50, u16(tg.turnPeak, 10), true); dv.setUint8(52, tg.steps || 0); dv.setUint8(53, 0);
    dv.setUint16(54, u16(tg.cadence, 10), true); dv.setUint16(56, u16(tg.stepCv, 10), true); dv.setUint16(58, u16(tg.speed, 100), true);
  ...
  var KIND_NM = { ftsst: "ลุกนั่ง 5 ครั้ง", tug: "ลุกเดิน 3 เมตร", balance: "ทรงตัว" };
  ...
  function open(opts) {
    ...
    function save(res) { fin({ kind: kind, sec: Math.round(res.totalMs / 100) / 10, reps: res.ftsst ? res.ftsst.reps : undefined, incomplete: res.status !== "ok", imu: res, raw: csv(), engine: ENGINE, endedBy: "imu", quality: qual(res),
    ...
    function startCd() {
      ...
      CD = setInterval(async function () { n--; if (n > 0) { setN(n, ""); speak(n === 2 ? "สอง" : "หนึ่ง"); return; } clearInterval(CD); CD = null; setN("0.0", " วินาที"); speak("เริ่ม");
        ...
        buttons([{ t: kind === "balance" ? "■ หยุดเพื่อความปลอดภัย" : "■ สิ้นสุด (นั่งลงแล้วระบบไม่หยุด)", cls: "no", fn: async function () { try { await cmd(CMD.STOP); } catch (e) {} if (twin) stop(twin, Date.now()); setTimeout(function () { if (phase === "run") done(twin && twin.result ? twin.result : { kind: kind, engine: ENGINE, status: "incomplete", totalMs: elapsed() * 1000 }); }, 600); } }]);
        tick = setInterval(function () { if (phase !== "run") return; var el = elapsed(); if (kind === "balance") { setN(Math.max(0, P.BAL_SEC - el).toFixed(0), " วินาที"); gauge(el / P.BAL_SEC * 100); } else setN(el.toFixed(1), " วินาที"); }, 100);
      }, 1000);
    }
    BLE.onState = function (s) {
      ...
      if (phase === "run") { if (kind !== "balance") gauge(clamp(s.tilt / 45 * 100, 0, 100)); if (s.ev === EV.STAND) speak(kind === "ftsst" ? String(s.count) : "เดินไปได้เลย"); if (s.ev === EV.TURN && kind === "tug") speak("เดินกลับมานั่ง"); if (s.ev === EV.IMPACT) speak("ระวัง"); }
    };
  ...
  g.CSImu = Object.assign({ supported: supported, isiOS: isiOS, connect: connect, disconnect: disconnect, cmd: cmd, open: open, close: close, isOpen: function () { return !!UI; }, fw: function () { return BLE.fw; } }, PURE);
